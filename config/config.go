...
type Access struct {
	// Enable decides whether use access token
...
type ServiceCert struct {
	// Enable decides whether use service cert
	Enable bool `yaml:"enable"`
	...
	// IntermediateCert decides whether concatinate intermediate cert to end-entity cert
	IntermediateCert bool `yaml:"intermediate_cert"`

	// PrincipalAuthHeaderName is the HTTP header name for holding the n-token.
	PrincipalAuthHeaderName string `yaml:"auth_header_key"`

	// Spiffe decides whether include spiffe or not
